...
function govcms_field_widget_form_alter(&$element, FormStateInterface $form_state, $context) {
  ...
  // In Drupal 7 `hook_field_widget_form_alter` could be implemented by the theme.
  // but in Drupal 8 this hook changed to being module-only. This hook can be
  // critical for improving the editor experience for things like paragraphs.
  // Implementing themes can access the plugin ID via $context['widget']->getPluginId().
