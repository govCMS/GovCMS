...
describe('User can create '+content_type+' content', () => {
  ...
  it('Log the user in then create a '+content_type, () => {
    ...
    cy.userLogin(role1).then(() => {
      ...
      cy.intercept('POST', Cypress.config('baseUrl')+'/node/add/govcms_'+content_type.toLowerCase()+'?ajax_form=1&_wrapper_format=drupal_ajax').as('media_window')
      ...
      .wait('@media_window').then(() =>{
        ...
        .then(() => {
          ...
          .click({force: true}).wait('@media_window').then(() => {
            ...
            cy.get('#edit-field-more-information-0-uri').type('<front>')
            cy.get('#edit-field-more-information-0-title').type('A link title')
            cy.get('#edit-submit')
            .click()
