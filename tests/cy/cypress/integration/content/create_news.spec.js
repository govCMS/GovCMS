...
const content_type = 'News and Media'
...
describe('User can create '+content_type+' content', () => {
  ...
  it('Log the user in then create a '+content_type, () => {
    ...
    cy.userLogin(role1).then(() => {
      ...
      cy.intercept('POST', Cypress.config('baseUrl')+'/node/add/govcms_news_and_media?ajax_form=1&_wrapper_format=drupal_ajax').as('media_window')
      ...
      .wait('@media_window').then(() =>{
        ...
        .then(() => {
          ...
          .click({force: true}).wait('@media_window').then(() => {
            ...
            cy.get('#edit-field-media-release-type')
            .select('News')
            cy.get('#edit-field-published-date-time-0-value-date')
            .type(yyyy+'-'+mm+'-'+dd)
            cy.get('#edit-field-published-date-time-0-value-time')
            .type(hour+':'+min+':'+sec)
            cy.get('#edit-submit')
            .click()
